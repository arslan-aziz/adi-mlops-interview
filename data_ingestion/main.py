...
def make_nasa_api_search_request_and_get_json(page_id: int) -> dict:
    ...
    nasa_api_client = NasaImageApiClient(
        base_url=NASA_IMAGE_API_BASE_URL, api_key=NASA_API_KEY
    )
    try:
...
def parse_nasa_image_api_search_hit(response_data: dict) -> dict:
    ...
    date_created_dt = datetime.datetime.strptime(
        response_data["data"][0]["date_created"], dt_format
...
def main() -> None:
    ...
    images_parsed_rdd = (
        pages_rdd.map(
            lambda page_id: make_nasa_api_search_request_and_get_json(page_id)
        )
        .flatMap(lambda x: x)
    ...
    images_parsed_df = spark.createDataFrame(
        images_parsed_rdd, NASA_IMAGE_API_PAGE_PARSED_SPARK_DF_SCHEMA
    )

    # run udf to download images
    download_bytes_udf = udf(download_bytes, BinaryType())
    images_parsed_df = images_parsed_df.withColumn(
        "image_bytes", download_bytes_udf(col("image_href"))
    )

    # format the output url of the images
    images_prefix = (
        "s3://"
        + os.path.join(S3_BUCKET_NAME, S3_KEY_PREFIX, S3_IMAGES_OUTPUT_LOCATION)
        + "/"
    )
    logger.warning(f"Images will be uploaded to {images_prefix}")
    images_parsed_df = images_parsed_df.withColumn(
        "image_s3_url",
        concat(lit(images_prefix), col("nasa_id"), col("image_file_extension")),
    )

    # split images and records since they will be written separately
    images_df = images_parsed_df.select(
        col("image_s3_url").alias("s3_url"), col("image_bytes").alias("data")
    )
    records_df = images_parsed_df.select(
        "nasa_id",
        "title",
        "description",
        "date_created",
    ...
    records_location = "s3://" + os.path.join(
        S3_BUCKET_NAME, S3_KEY_PREFIX, S3_RECORDS_OUTPUT_LOCATION
    )
